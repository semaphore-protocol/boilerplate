...
task("deploy:reviews", "Deploy a Reviews contract")
    ...
    .setAction(async ({ logs, treeDepth, verifierAddress }, { ethers }) => {
        ...
        const FactoryContract = await ethers.getContractFactory("Reviews", {
            libraries: {
        ...
        if (logs) {
            console.info(`Reviews contract has been deployed to: ${contract.address}`)
        }
